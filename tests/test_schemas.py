...
def test_bug_create_schema(fakepublish, bug_create_message, testrelay):
    ...
    assert message.component_name == "smplayer"
    assert message.product_name == "Fedora"
    assert (
        message.summary
        == "dgunchev filed a new bug RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
    assert (
        str(message)
        == "dgunchev filed a new bug RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
    assert message.url == "https://bugzilla.redhat.com/show_bug.cgi?id=2436734"
    assert (
    ...
    assert message.packages == ["smplayer"]
    assert (
...
def test_bug_modify_schema(fakepublish, bug_modify_message, testrelay):
    ...
    assert (
        message.summary
        == "mhroncok@redhat.com updated 'cc' on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
    assert message.usernames == ["adamw", "davide", "upstream-release-monitoring"]
...
def test_bug_modify_four_changes_schema(fakepublish, bug_modify_message_four_changes, testrelay):
    ...
    assert (
        message.summary
        == "zebob.m@gmail.com updated 'assigned_to', 'bug_status', 'cc', and 'flag.needinfo' on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
    assert message.usernames == ["davide"]
...
def test_bug_modify_two_changes_schema(fakepublish, bug_modify_message_four_changes, testrelay):
    ...
    bug_modify_message_four_changes["event"]["changes"] = bug_modify_message_four_changes["event"][
        "changes"
    ][:2]
    testrelay.on_kafka_message(bug_modify_message_four_changes)
    assert fakepublish.call_count == 1
    ...
    assert (
        message.summary
        == "zebob.m@gmail.com updated 'assigned_to' and 'bug_status' on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
...
def test_bug_modify_no_changes_schema(fakepublish, bug_modify_message, testrelay):
    ...
    del bug_modify_message["event"]["changes"]
    testrelay.on_kafka_message(bug_modify_message)
    assert fakepublish.call_count == 1
    message = fakepublish.call_args[0][0]
    # this should not raise an exception
    ...
    assert (
        message.summary
        == "mhroncok@redhat.com updated something unknown on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
    assert message.usernames == ["davide", "upstream-release-monitoring"]
...
def test_comment_create_schema(fakepublish, comment_create_message, testrelay):
    ...
    assert (
        message.summary
        == "rjones added comment on RHBZ#2436732 'libguestfs appliance fails to auto-assem...'"
    )
...
def test_attachment_create_schema(fakepublish, attachment_create_message, testrelay):
    ...
    assert (
        message.summary
        == "peter added attachment on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
...
def test_attachment_modify_schema(fakepublish, attachment_modify_message, testrelay):
    ...
    assert (
        message.summary
        == "joe updated 'isobsolete' for attachment on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
...
def test_attachment_modify_no_changes_schema(fakepublish, attachment_modify_message, testrelay):
    ...
    # wipe the 'changes' dict from the sample message, to simulate
    # one of these broken messages
    del attachment_modify_message["event"]["changes"]
    testrelay.on_kafka_message(attachment_modify_message)
    assert fakepublish.call_count == 1
    message = fakepublish.call_args[0][0]
    # this should not raise an exception
    ...
    assert (
        message.summary
        == "joe updated something unknown for attachment on RHBZ#2436734 '[abrt] smplayer: qt_message_fatal(): smp...'"
    )
...
def test_component_not_package_schema(fakepublish, bug_create_message, testrelay):
    ...
    # filter out
    bug_create_message["bug"]["component"]["name"] = "distribution"
    testrelay.on_kafka_message(bug_create_message)
    assert fakepublish.call_count == 1
    message = fakepublish.call_args[0][0]
    # this should not raise an exception
...
def test_bug_no_qa_contact(fakepublish, bug_create_message, testrelay):
    ...
    bug_create_message["bug"]["qa_contact"] = None
    testrelay.on_kafka_message(bug_create_message)
    assert fakepublish.call_count == 1
    message = fakepublish.call_args[0][0]
    # this should not raise an exception
